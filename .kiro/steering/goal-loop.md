...
The goal contract and its check decide when the task is done. You do not get to declare success because the code looks correct, the diff seems plausible, or the prose summary sounds complete. The stop hook is in charge because it reruns the check after each finished turn.
