...
// format goal-loop already uses, with one difference: `description` must be a
// single line under 160 chars. The canonical descriptions are long (tuned for
// Cursor's skill picker), so each ships a short one here. The body is copied
...
// tests fail if the committed copies are stale.

const fs = require('fs');
const path = require('path');

const ROOT = path.join(__dirname, '..');
const HOMEPAGE = 'https://github.com/bodecloud/goal-loop';

const DESCRIPTIONS = {
  'cursor-goal': 'Work toward the active Goal Loop objective until a shell check passes. Supports check-backed loops, pause/resume, blocked state, and progress tracking.',
};
